...
public class CuttingPlane {
	...
	public static int n, m, Delta, no_cuts = 0, no_cliqueCuts = 0, no_lazyConstraints = 0, no_S_colorCuts = 0,
			no_blockCuts = 0, no_2_rankCuts = 0, step = 0, max_steps = 1000, kClique = 7, verbosity = 0,
			chi_UpperBound = n;
	public static long timeout = 10000, minTimeUp, maxTimeUp, minTimeDown, maxTimeDown, avgTime;
	public static boolean[][] adjacency, M_adjacency;// adjacency matrix
	public static int[] degrees, M_degrees, vOrder, revOrder;
	public static double currentObj = -1, precisionRC = 1e-5, precisionVar = 1e-7, precisionCut = 1e-7,
			precisionOpt = 1e-9, precisionLB = 1e-5, rootLowerB = -1e20, timeLimit = 4;
	public static GRBModel M_model;
	public static double[][] solutionX;
	public static double[] solutionW;
	...
	public static void readFile(String dataFile) {
		...
		degrees = new int[n];
		Delta = readGfF.readGraph(dataFile, M_adjacency, adjacency, degrees, Delta);
	...
	public static int sumDegrees(ColorClass clique) {
		int sum = 0;
		Iterator<Integer> iterClique = clique.vertices.iterator();
		while (iterClique.hasNext())
			sum += degrees[iterClique.next()];
		return sum;
	}

	public static ColorClass M_cliqueBuild() {
		int v, x, max = 0;
		boolean toAdd = true;
		Iterator<Integer> iterV, iterClique;
		ColorClass maxClique = new ColorClass(), currentClique;
		ArrayList<Integer> indexList = M_orderDegrees();
		for (int u = 0; u < n; u++) {
			v = indexList.remove(0);
			indexList.add(v);
			currentClique = new ColorClass();
			currentClique.vertices.add(v);
			iterV = indexList.iterator();
			while (iterV.hasNext()) {
				x = iterV.next();
				toAdd = true;
				iterClique = currentClique.vertices.iterator();
				while (iterClique.hasNext())
					if (!adjacency[iterClique.next()][x]) {
						toAdd = false;
						break;
					}
				if (toAdd)
					currentClique.vertices.add(x);
			}
			if (currentClique.vertices.size() == max && sumDegrees(currentClique) > sumDegrees(maxClique)) {
				// System.out.println("Current: " + sumDegrees(currentClique));
				// System.out.println("Max: " + sumDegrees(maxClique));
				max = currentClique.vertices.size();
				maxClique = currentClique;
			}
			if (currentClique.vertices.size() > max) {
				max = currentClique.vertices.size();
				maxClique = currentClique;
			}
		}
	...
	public static void M_newOrder() {
		// TODO:?!
		ColorClass maxClique = M_cliqueBuild();
		int s = maxClique.vertices.size(), t = 0, p;
		int[] newDegrees = new int[n - s];
		vOrder = new int[n];
		revOrder = new int[n];
		Integer[] indices = new Integer[n - s];
		boolean[] chClique = new boolean[n];
		Iterator<Integer> iterClique = maxClique.vertices.iterator();
		while (iterClique.hasNext()) {
			p = iterClique.next();
			vOrder[t] = p;
			t++;
		...
		for (int i = 0; i < n; i++)
			if (!chClique[i]) {
				newDegrees[t] = degrees[i];
				indices[t] = i;
				t++;
			}
		Arrays.sort(indices, Comparator.comparingInt(i -> degrees[i]));
		for (int i = s; i < n; i++) {
			vOrder[i] = indices[n - i - 1];
			revOrder[indices[n - i - 1]] = i;
		}
		// change the order:
		boolean[][] newAdjacency = new boolean[n][n];
		int[] nDegrees = new int[n];
		for (int i = 0; i < n; i++) {
			nDegrees[i] = degrees[vOrder[i]];
			for (int j = 0; j < n; j++) {
				newAdjacency[i][j] = adjacency[vOrder[i]][vOrder[j]];
				newAdjacency[j][i] = adjacency[vOrder[j]][vOrder[i]];
			}
		}
			...
			for (int s = 2; s < t - 1; s++)
				if (checkTime(totalTime)) {
					...
					if (cutGap > 0) {
						...
						// System.out.println("New S-cut; gap: " + cutGap);
						// (new ColorClass(best_S_list)).toString();
	...
	public static void M_buildInitialModelO(GRBEnv env) throws GRBException {
		...
		// To be added as cuts (lazy constraints):
		/*
		 * for (int v = 1; v < n; v++) for (int i = 1; i <= v; i++) { lin_expr = new
		 * GRBLinExpr(); lin_expr.addTerm(-1.0, model.getVarByName("x_" + v + "_" + i));
		 * for (int u = i - 1; u < v; u++) lin_expr.addTerm(1.0, model.getVarByName("x_"
		 * + u + "_" + (i - 1))); model.addConstr(lin_expr, GRB.GREATER_EQUAL, 0.0,
		 * "Sort_" + v + "_" + i); }
		 */
	...
	public static void main(String[] args) throws GRBException, IOException, InterruptedException {
		// M_model.write(path + "/results/" + fileName + "/model_cuts.lp");
